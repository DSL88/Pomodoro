document.addEventListener('DOMContentLoaded', () => {
  // Estado da aplicação
  let tasks = []; // Será carregado após inicializar sessão
  ...
  // Sistema de reconhecimento de IP e gestão de sessões
  let userIP = null;
  ...
  async function getUserIP() {
    try {
      ...
      for (const service of ipServices) {
        try {
          console.log(`🌐 Tentando obter IP de: ${service}`);
      ...
      // Fallback: usar um identificador único baseado em características do navegador
      console.log('⚠️ Não foi possível obter IP, usando fingerprint do navegador');
  ...
  // Gerar fingerprint único do navegador como fallback
  function generateBrowserFingerprint() {
    const canvas = document.createElement('canvas');
    const ctx = canvas.getContext('2d');
    ctx.textBaseline = 'top';
    ctx.font = '14px Arial';
    ctx.fillText('Pomodoro fingerprint', 2, 2);
  ...
  async function initializeUserSession() {
    console.log('🚀 Inicializando sessão do usuário...');
    
    try {
      userIP = await getUserIP();
      currentSession = `pomodoro_session_${userIP}`;
      
      console.log(`👤 Sessão do usuário: ${currentSession}`);
      
      // Verificar se é um novo usuário
      const existingData = localStorage.getItem(currentSession);
      if (!existingData) {
        console.log('🆕 Novo usuário detectado - criando sessão limpa');
        showWelcomeMessage();
        // Inicializar com dados vazios
        saveTasksForSession([]);
      } else {
        console.log('👋 Usuário existente - carregando dados salvos');
      ...
      currentSession = 'pomodoro_default_session';
      loadTasksFromSession();
    }
  }

  // Salvar tarefas para a sessão específica
  function saveTasksForSession(tasksArray = tasks) {
    try {
      const sessionData = {
        tasks: tasksArray,
      ...
      console.log(`💾 Dados salvos para sessão: ${currentSession}`);
    } catch (error) {
      console.error('❌ Erro ao salvar dados da sessão:', error);
    }
  }

  // Carregar tarefas da sessão específica
  function loadTasksFromSession() {
    try {
      ...
      if (savedData) {
        ...
        tasks = [];
        console.log('📂 Nenhuma tarefa encontrada - iniciando com lista vazia');
      }
    } catch (error) {
    ...
    welcomeNotification.innerHTML = `
      <div class="welcome-content">
        ...
        <div class="welcome-text">
          <h3>Bem-vindo ao Pomodoro!</h3>
          <p>Esta é sua área pessoal de produtividade. Suas tarefas serão salvas automaticamente.</p>
        </div>
  ...
  function migrateLegacyData() {
    try {
      ...
      if (oldTasks && !localStorage.getItem(currentSession)) {
        console.log('🔄 Migrando dados antigos para nova sessão...');
        const legacyTasks = JSON.parse(oldTasks);
        saveTasksForSession(legacyTasks);
        // Manter dados antigos como backup
        localStorage.setItem('tasks_backup', oldTasks);
        console.log('✅ Migração concluída');
      }
    } catch (error) {
      console.warn('⚠️ Erro na migração de dados antigos:', error);
    }
  }

  // Adicionar indicador de sessão na página
  function addSessionIndicator() {
    ...
    indicator.innerHTML = `
      <div class="session-info">
        ...
        <span class="session-text">Sessão: ${userIP ? userIP.substring(0, 8) + '...' : 'Local'}</span>
        <span class="session-tasks">${tasks.length} tarefa${tasks.length !== 1 ? 's' : ''}</span>
      </div>
    `;
  ...
  function updateSessionIndicator() {
    ...
    if (indicator) {
      const tasksSpan = indicator.querySelector('.session-tasks');
      if (tasksSpan) {
        tasksSpan.textContent = `${tasks.length} tarefa${tasks.length !== 1 ? 's' : ''}`;
      }
    }
  }
  ...
  async function initializeApp() {
    console.log('🚀 Inicializando aplicação Pomodoro...');
    
    // Inicializar sessão do usuário primeiro
    ...
    renderTasks();
    resetTimer();
    addSessionIndicator(); // Adicionar indicador de sessão
    updateSessionIndicator(); // Atualizar indicador de sessão
